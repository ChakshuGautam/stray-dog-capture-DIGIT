...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│ ⚙️ System Configuration                                    [Save All]       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  Last Updated: Dec 7, 2026 10:30 AM by Admin (Rajesh K.)                   │
│  Pending Approvals: 2                                                       │
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Parameter              Current    New Value   Effective Date           ││
│  │  ─────────────────────────────────────────────────────────────────     ││
│  │                                                                         ││
│  │  Payout per Capture     ₹500       [₹500    ]  [Immediate ▼]           ││
│  │  Info: Amount paid to teacher when dog is successfully captured         ││
│  │                                                                         ││
│  │  Payout per Owner Claim ₹250       [₹250    ]  [Immediate ▼]           ││
│  │  Info: Amount paid when owner claims the dog                            ││
│  │                                                                         ││
│  │  Payout per UTL Resolve ₹100       [₹100    ]  [Immediate ▼]           ││
│  │  Info: Amount paid when UTL case is later resolved                      ││
│  │                                                                         ││
│  │  Monthly Cap per Teacher ₹5,000    [₹5,000  ]  [Immediate ▼]           ││
│  │  Info: Maximum monthly payout per teacher                               ││
│  │                                                                         ││
│  │  ⚠ Pending Change: Monthly Cap → ₹6,000 (Effective: Jan 1, 2025)       ││
│  │    Submitted by: Admin A | Awaiting approval from: Admin B              ││
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Parameter                 Current    New Value    Sensitivity          ││
│  │  ─────────────────────────────────────────────────────────────────     ││
│  │                                                                         ││
│  │  GPS Accuracy Threshold    10m       [10    ] m   [Standard ▼]         ││
│  │  Info: Max distance between photo GPS and teacher's location            ││
│  │                                                                         ││
│  │  Photo Similarity Score    85%       [85    ] %   [Standard ▼]         ││
│  │  Info: Threshold for flagging duplicate photos                          ││
│  │                                                                         ││
│  │  Max Submissions/Day       10        [10    ]     [Standard ▼]         ││
│  │  Info: Maximum submissions per teacher per day                          ││
│  │                                                                         ││
│  │  Anomaly Detection Level   Medium    [Medium ▼]                        ││
│  │  Info: Low=fewer alerts, High=more sensitive                            ││
│  │                                                                         ││
│  │  Risk Score Threshold      70        [70    ]     [Standard ▼]         ││
│  │  Info: Score above this triggers fraud review                           ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Parameter                   Current    New Value   Info                ││
│  │  ─────────────────────────────────────────────────────────────────     ││
│  │                                                                         ││
│  │  Rejection Rate Warning      20%       [20   ] %   First warning        ││
│  │  Rejection Rate Suspension   30%       [30   ] %   Temp suspension      ││
│  │  Rejection Rate Ban          50%       [50   ] %   Permanent ban        ││
│  │                                                                         ││
│  │  Min Submissions for Penalty 10        [10   ]     Minimum sample       ││
│  │                                                                         ││
│  │  Suspension Duration         7 days    [7    ] d   Temp suspension      ││
│  │  Appeal Window               30 days   [30   ] d   Time to appeal       ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Alert Type                  Enabled   Threshold   Recipients           ││
│  │  ─────────────────────────────────────────────────────────────────     ││
│  │                                                                         ││
│  │  Budget Threshold Alert      [✓]       [85   ] %   State Admins        ││
│  │  High Fraud Rate Alert       [✓]       [0.5  ] %   State + District    ││
│  │  Submission Spike Alert      [✓]       [300  ] %   District Admins     ││
│  │  MC Backlog Alert            [✓]       [7    ] d   State + MC Mgr      ││
│  │  System Health Alert         [✓]       [95   ] %   Tech Team           ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
│  │  ─────────────────────────────────────────────────────────────────     ││
│  │                                                                         ││
│  │  Daily Report Generation     [02:00   ] AM                              ││
│  │  Leaderboard Update          [00:00   ] (Midnight)                      ││
│  │  Data Retention Period       [5       ] years                           ││
│  │  Session Timeout             [30      ] minutes                         ││
│  │  Max Login Attempts          [5       ]                                 ││
│  │                                                                         ││
│  │  Maintenance Window          [Sunday 2:00 AM - 4:00 AM        ]        ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Based on current pending changes:                                      ││
│  │                                                                         ││
│  │  • Monthly Cap ₹5,000 → ₹6,000                                         ││
│  │    - Estimated monthly budget impact: +₹12.4L                          ││
│  │    - Teachers affected: 2,340 (who hit current cap)                    ││
│  │    - Expected payout increase per teacher: ~₹530/month                 ││
│  │                                                                         ││
...
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │  📜 Configuration History                                [View All]    ││
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Date         Parameter          Old      New      Changed By           ││
│  │  ─────────────────────────────────────────────────────────────────     ││
│  │  Dec 1, 2026  Monthly Cap        ₹4,500   ₹5,000   Admin (Priya S.)    ││
│  │  Nov 15, 2026 GPS Threshold      15m      10m      Admin (Rajesh K.)   ││
│  │  Nov 1, 2026  Payout per Capture ₹450     ₹500     Admin (Priya S.)    ││
│  │  ...                                                                    ││
│  │                                                                         ││
│  │  [Export History] [Rollback Change]                                    ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│          [Save Draft] [Submit for Approval] [Reset to Defaults]             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
class SystemConfigService {
  ...
  static validatePayoutConfig(config) {
    ...
    if (config.perCapture < 0 || config.perCapture > 10000) {
      errors.push('Payout per capture must be between ₹0 and ₹10,000');
    }
  ...
  return (
    <div className="pending-change-alert">
      ...
      <div className="alert-content">
        <strong>Pending Change:</strong> {change.parameter} → {change.newValue}
