...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│ 💰 State Budget Utilization                              FY 2026-25         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│  ┌────────────────────┐ ┌────────────────────┐ ┌────────────────────┐      │
│  │  ALLOCATED BUDGET  │ │  UTILIZED          │ │  REMAINING         │      │
│  │    ₹25.00 Cr       │ │    ₹18.45 Cr       │ │    ₹6.55 Cr        │      │
│  │   100%             │ │   73.8%            │ │   26.2%            │      │
│  └────────────────────┘ └────────────────────┘ └────────────────────┘      │
│                                                                             │
...
│  │                                                                         ││
│  │   Monthly Avg: ₹2.05 Cr    Projected Year-End: ₹24.60 Cr               ││
│  │   Status: ✓ On Track (within budget)                                   ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │  📍 District Budget Utilization                    [Sort: Util % ▼]    ││
│  ├──────────────┬───────────┬───────────┬────────────┬───────────┬────────┤│
│  │ District     │ Allocated │ Utilized  │ Util %     │ Remaining │ Status ││
│  ├──────────────┼───────────┼───────────┼────────────┼───────────┼────────┤│
│  │ Patna        │ ₹1.50 Cr  │ ₹1.32 Cr  │    88% ⚠   │ ₹0.18 Cr  │ At Risk││
│  │ Gaya         │ ₹1.20 Cr  │ ₹0.95 Cr  │    79%     │ ₹0.25 Cr  │ Normal ││
│  │ Muzaffarpur  │ ₹1.00 Cr  │ ₹0.72 Cr  │    72%     │ ₹0.28 Cr  │ Normal ││
│  │ Bhagalpur    │ ₹0.90 Cr  │ ₹0.68 Cr  │    76%     │ ₹0.22 Cr  │ Normal ││
│  │ Darbhanga    │ ₹0.85 Cr  │ ₹0.78 Cr  │    92% ⚠   │ ₹0.07 Cr  │ Critical│
│  │ ...          │           │           │            │           │        ││
│  │ Total        │ ₹25.00 Cr │ ₹18.45 Cr │    74%     │ ₹6.55 Cr  │        ││
│  └──────────────┴───────────┴───────────┴────────────┴───────────┴────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
│  │  💵 Payout Breakdown                                                   ││
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  🔴 Darbhanga: 92% utilized, only ₹7L remaining (2 months left)        ││
│  │     Projected shortfall: ₹8L                                           ││
│  │     [Request Reallocation] [View Details]                              ││
│  │                                                                         ││
│  │  🟡 Patna: 88% utilized, ₹18L remaining                                ││
│  │     On track if current rate maintained                                ││
│  │     [View Details]                                                     ││
│  │                                                                         ││
│  │  🟢 32 districts within normal budget utilization                      ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│  ┌─────────────────────────────────────────────────────────────────────────┐│
...
│  ├─────────────────────────────────────────────────────────────────────────┤│
│  │                                                                         ││
│  │  Current Monthly Burn Rate: ₹2.05 Cr                                   ││
│  │  Months Remaining: 3 (Jan - Mar)                                       ││
│  │  Projected Additional Spend: ₹6.15 Cr                                  ││
│  │  Projected Year-End Total: ₹24.60 Cr                                   ││
│  │                                                                         ││
│  │  Budget Status: ✓ Within allocation (₹0.40 Cr buffer)                  ││
│  │                                                                         ││
│  │  Scenarios:                                                             ││
│  │  • Conservative (+10% growth): ₹25.06 Cr ⚠ Slight Overrun             ││
│  │  • Aggressive (+20% growth): ₹25.68 Cr 🔴 Overrun                      ││
│  │  • Reduced (-10% growth): ₹24.14 Cr ✓ Under Budget                     ││
│  │                                                                         ││
│  └─────────────────────────────────────────────────────────────────────────┘│
│                                                                             │
│      [Export Financial Report] [Audit Trail] [Request Budget Adjustment]    │
└─────────────────────────────────────────────────────────────────────────────┘
```
    ...
    <div className="budget-dashboard">
      <header className="dashboard-header">
        ...
        <select value={fiscalYear} onChange={(e) => setFiscalYear(e.target.value)}>
          <option value="2026-25">FY 2026-25</option>
          ...
          {monthlyData && (
            <div className="monthly-stats">
              ...
              <span className={projections?.withinBudget ? 'status-ok' : 'status-warning'}>
                {projections?.withinBudget ? '✓ On Track' : '⚠ Over Budget'}
              </span>
            </div>
          )}
        </div>
      ...
      {alertDistricts.length > 0 && (
        <div className="alerts-section">
          ...
          <div className="normal-districts-summary">
            🟢 {districtBudgets.length - alertDistricts.length} districts within normal budget utilization
          </div>
        </div>
      )}
          ...
          <div className="projection-details">
            ...
            <div className={`budget-status ${projections.withinBudget ? 'ok' : 'warning'}`}>
              {projections.withinBudget
                ? `✓ Within allocation (${BudgetService.formatCurrency(projections.buffer)} buffer)`
                : `⚠ Over allocation by ${BudgetService.formatCurrency(projections.overrun)}`
              }
            </div>
          </div>
            ...
            {projections.scenarios.map((scenario, i) => (
              <div key={i} className={`scenario ${scenario.status}`}>
                <span className="scenario-icon">
                  {scenario.status === 'ok' ? '✓' : scenario.status === 'warning' ? '⚠' : '🔴'}
                </span>
